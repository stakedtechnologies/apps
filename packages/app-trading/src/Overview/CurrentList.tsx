...
function isFiltered (a: OfferOf, filter: OfferFilter, currentNumber: BlockNumber): boolean {
...
export default function CurrentList ({ allOffers }: Props): React.ReactElement<Props> | null {
  ...
  const { api } = useApi();
  const currentNumber = useCall<BlockNumber>(api.derive.chain.bestNumber, []);
  ...
  useEffect((): void => {
    if (allOffers) {
      const offers = filterOffers(allOffers, filter, nameFilter, favorites, currentNumber as BlockNumber);
      setOffers(offers);
    }
